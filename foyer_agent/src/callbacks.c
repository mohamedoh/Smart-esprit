...
msg=lookup_widget(button,"labelADD_MSG");
if (verif(s.ID)==0)
{
add_student(s);

strcpy(ch,"student added successfully " );
...
{
strcpy(ch,"student ID already exist " );
gtk_label_set_text(GTK_LABEL(msg),ch);
...
msg=lookup_widget(button,"labelREMOVE_MSG");
...
strcpy(ch1,"student removed successfully " );
...
{
strcpy(ch1,"student ID don't exist " );
gtk_label_set_text(GTK_LABEL(msg),ch1);
...
strcpy(s.FLOOR,gtk_entry_get_text(GTK_ENTRY(FLOOR)));
strcpy(s.CHAMBER,gtk_entry_get_text(GTK_ENTRY(CHAMBER)));
msg=lookup_widget(button,"labelUPDATE_MSG");
if (verif(s.ID)==1)
{
update_student(s,ch);

strcpy(ch1,"student updated successfully " );
gtk_label_set_text(GTK_LABEL(msg),ch1);
